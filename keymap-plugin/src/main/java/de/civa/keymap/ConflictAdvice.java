...
final class ConflictAdvice {
  ...
  enum Category {
    ...
    DELIBERATE("Works as-is — IntelliJ gets the key while focused");
  ...
  static {
    Advice windowManagement = new Advice(Category.DELIBERATE,
      "macOS uses this key to move or resize windows. IntelliJ receives it first while its window "
      + "is in front, so your IntelliJ shortcut keeps working. If you prefer the macOS window "
    ...
    BY_ID.put("MinimizeWindow", new Advice(Category.DELIBERATE,
      "Both macOS and IntelliJ minimize the window on ⌘M — the same thing on both sides, so there "
      + "is nothing to fix."));
    BY_ID.put("ShowContextualMenu", new Advice(Category.DELIBERATE,
      "macOS can open the context menu on this key, but IntelliJ receives it first while its "
      + "window is in front, so your IntelliJ shortcut keeps working. To free the key for macOS, "
      + "change it under System Settings → Keyboard → Keyboard Shortcuts → Keyboard."));

    Advice appWindowCycle = new Advice(Category.DELIBERATE,
      "IntelliJ switches between its project windows on this key, which is the same idea as the "
      + "macOS \"move focus between windows\" shortcut — and IntelliJ handles it first for its own "
    ...
    Advice spaces = new Advice(Category.RESOLVE,
      "macOS switches Spaces on this key across the whole system, so the IntelliJ shortcut never "
      + "fires. Give the IntelliJ action a different shortcut, or change the macOS one under "
    ...
    Advice missionControl = new Advice(Category.RESOLVE,
      "macOS uses this key for Mission Control across the whole system, so the IntelliJ shortcut "
      + "never fires. Give the IntelliJ action a different shortcut, or change the macOS one under "
      + "System Settings → Keyboard → Keyboard Shortcuts → Mission Control.");
    ...
    Advice desktopSwitch = new Advice(Category.RESOLVE,
      ...
      + "wins over IntelliJ. Give the IntelliJ action a different shortcut, change desktop "
      + "switching under System Settings → Keyboard → Keyboard Shortcuts → Mission Control, or "
      + "reach the IntelliJ action from the menu.");
    for (int i = 1; i <= 9; i++) {
    ...
    Advice keyboardAccess = new Advice(Category.RESOLVE,
      "macOS uses this key for keyboard navigation across the whole system, so the IntelliJ "
      + "shortcut never fires. Give the IntelliJ action a different shortcut, or turn the macOS "
    ...
    Advice screenshots = new Advice(Category.RESOLVE,
      "macOS takes a screenshot on this key across the whole system, so the IntelliJ shortcut "
      + "never fires. Give the IntelliJ action a different shortcut, or change screenshots under "
    ...
    BY_ID.put("ShowSpotlight", new Advice(Category.RESOLVE,
      "macOS opens Spotlight on this key, so the IntelliJ shortcut never fires. Give the IntelliJ "
    ...
    BY_ID.put("ShowFinderSearch", new Advice(Category.RESOLVE,
      "macOS opens a Finder search window on this key, so the IntelliJ shortcut never fires. Give "
      + "the IntelliJ action a different shortcut, or change it under System Settings → Keyboard → "
      + "Keyboard Shortcuts → Spotlight."));
    BY_ID.put("ShowNotificationCenter", new Advice(Category.RESOLVE,
      "macOS opens Notification Center on this key, so the IntelliJ shortcut never fires. Give the "
      + "IntelliJ action a different shortcut, or change it under System Settings → Keyboard → "
      + "Keyboard Shortcuts → Mission Control."));
    BY_ID.put("ToggleDockHiding", new Advice(Category.RESOLVE,
      "macOS shows or hides the Dock on this key, so the IntelliJ shortcut never fires. Give the "
      + "IntelliJ action a different shortcut, or change it under System Settings → Keyboard → "
      + "Keyboard Shortcuts → Launchpad & Dock."));
    BY_ID.put("ShowAccessibilityControls", new Advice(Category.RESOLVE,
      "macOS opens the Accessibility controls on this key, so the IntelliJ shortcut never fires. "
      + "Give the IntelliJ action a different shortcut, or change it under System Settings → "
    ...
    Advice inputSources = new Advice(Category.RESOLVE,
      ...
      + "input source) and wins over IntelliJ. Keep the IntelliJ shortcut and turn the macOS one "
    ...
    BY_STROKE.put(KeyStroke.getKeyStroke(KeyEvent.VK_8, InputEvent.ALT_DOWN_MASK | InputEvent.META_DOWN_MASK),
      new Advice(Category.RESOLVE,
        ...
        + "Accessibility → Zoom, or give the IntelliJ action a different shortcut."));
  }
  ...
  static Advice resolve(List<ConflictScan.SystemShortcut> macOs, KeyStroke stroke) {
    ...
    return best != null ? best : new Advice(Category.UNCLASSIFIED,
      ...
      + "before IntelliJ — so the bound action may quietly never fire (⌘Escape behaves this way). "
      + "Give the IntelliJ action a different shortcut, or find and turn off the macOS shortcut "
