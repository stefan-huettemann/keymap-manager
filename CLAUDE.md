...
- `keymap-plugin/` — Gradle project: `plugin.xml` + the keymap XML, wired via the `bundledKeymap` extension point, plus five Java classes: `NationalLayoutCheck` warns (with a one-click fix) when the keymap is selected while JBR national keyboard layout support (`com.sun.awt.use.national.layouts`, default on for macOS) is disabled, without which the German-key bindings never fire; `ConflictScan` + `ConflictAdvice` + `ConflictReportDialog` implement the live keymap-conflict report, opened by `ShowConflictReportAction` (Tools → Review Keymap Conflicts, and the activation notification). Conflict detection is runtime (JBR `com.jetbrains.SystemShortcuts` via reflection, same source as IDEA's own conflict banner; in-keymap duplicates via public `Keymap` API); external conflicts are split by ownership (bindings this keymap declares vs. plugin/IDE bindings) by parsing the bundled keymap resource; curated per-conflict advice lives in `ConflictAdvice` (keep in sync with `keys.md` Appendix C). Keep the code surface at this set; the plugin's job is the keymap and its conflict report, not further features.
